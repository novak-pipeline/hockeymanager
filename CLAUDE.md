...
Special teams (#175) are now real: the GM edits his own PP1/PP2 + PK1/PK2 units on the Tactics screen (the sim reads `lines.powerPlayUnits`/`penaltyKillUnits`); shorthanded **scoring** is credited (`strength:'sh'` → shGoals/shAssists, real pk points); and PP/PK **time-on-ice** is tracked — the full sim splits each shift by strength (deployKey), the quick sim estimates PP/PK minutes from penalty volume + unit membership (SH/SHP + PP-TOI/PK-TOI columns on the stats table). Plus/minus tracked by both engines. 3D uses procedural primitives pending Blender glTF assets. (Cap credibility fixed #176 — generated rosters now run a realistic ~78–99% of the cap with a top-heavy salary shape; no more over-cap or ~40% skeleton payrolls.)
