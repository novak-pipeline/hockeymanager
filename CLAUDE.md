...
Special teams (#175) are now real: the GM edits his own PP1/PP2 + PK1/PK2 units on the Tactics screen (the sim reads `lines.powerPlayUnits`/`penaltyKillUnits`); shorthanded **scoring** is credited (`strength:'sh'` → shGoals/shAssists, real pk points); and both sims deploy the real PP/PK units on special teams — the full sim always did; the quick sim now sends PP1/PK1 out during power plays/kills (so PP1 collects the power-play goals, not a random line) and credits PP/PK **time-on-ice** literally (SH/SHP + PP-TOI/PK-TOI columns on the stats table). Plus/minus tracked by both engines. 3D uses procedural primitives pending Blender glTF assets. (Cap credibility fixed #176 — generated rosters now run a realistic ~78–99% of the cap with a top-heavy salary shape; no more over-cap or ~40% skeleton payrolls.)
