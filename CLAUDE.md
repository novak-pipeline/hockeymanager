...
Known tuning debt: PP/PK **time-on-ice** splits are still placeholders (0 in the pp/pk situation stats) — but the shorthanded **scoring** side is now real (#175: `strength:'sh'` goals credited to shGoals/shAssists; archived seasons write a true pk split; SH/SHP columns on the profile + stats table). Plus/minus tracked by both sim engines. 3D uses procedural primitives pending Blender glTF assets. (Cap credibility fixed #176 — generated rosters now run a realistic ~78–99% of the cap with a top-heavy salary shape; no more over-cap or ~40% skeleton payrolls.)
