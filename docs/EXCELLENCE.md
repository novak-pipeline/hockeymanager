...
6. **[RE-SCOPED 2026-07-28]** ~~Chore-ectomy of training/practice: weekly nag → monthly plan~~ — **the nag no longer exists.** Verified in code: `tickPractice` runs every 7 days silently (fatigue + promise upkeep), pushes no inbox item, sets no due-flag and gates nothing; practice reaches the GM only through the bi-weekly staff meeting, which is already a beat gate with a delegate escape, plus two opt-in screens. Task #170 appears to have removed the chore already. **What remains is the last clause only: visible payoff receipts.** A grep across the codebase for any payoff/attribution concept returns nothing — the Development Center lets you set a focus and says it "biases his growth", but nothing ever tells you whether it worked. That is a build task (needs per-player attribution of growth to focus, persisted), not a diagnosis. *(B2.4)*
