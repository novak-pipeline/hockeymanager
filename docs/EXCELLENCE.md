...
4. **[CLOSED 2026-07-31]** In-season cap compliance (known filed gap — emergencyRecalls stacks salary uncapped). *(B7.3)* — closed in two passes. 2026-07-28 shut the discretionary `callUp` path; the 07-31 autopilot then found the club still $6.5M over ($98.5M vs a $92.0M ceiling), because the trades the GM PROPOSED HIMSELF (`executeUserTrade`) were the one trade path with no cap check — incoming offers were guarded, outgoing ones weren't. Guarded at both ends now (fast-no at propose time, hard check at execution) and the same arithmetic drives the AGM's read, the Accept button and the generators. Autopilot on seed 2029: 4 cap majors → 0. Emergency recalls still deliberately eat an overage to ice a lineup (`callUpCap.test.ts`).
5. **[BLOCKED — user]** Release trust: code-signing/SmartScreen, dev-freeze fix, voices in packaged build. *(B1.4)*
5b. **[MEASURED 2026-07-31]** **Lever audit (#154) — every visible lever proven real or cut.** *(B7.3)* — the whole surface measured, not asserted: 40,000 mirror games per lever, mirrored benches, published in [LEVER-AUDIT.md](LEVER-AUDIT.md). **REAL:** line assembly (**+24.1 pts/82**), PP1 (+6.5), PK1 (+4.9), goalie order (+4.3), coach PP/PK edge (+3.4 each), coach fit (+2.5); and the man-management channel — freshness (**+37.9**), form (+17.5), morale (+11.3). **THREE DEFECTS FOUND AND FIXED:** the healthy-scratch toggle never took the player off the ice; the team practice regimen reached only NHL players (2.2 pts of headroom, +0.07 effect) and not the farm (11.4 pts, +0.5) — now org-wide; and the `passing` slider was a live **−26 pts/season coach-hiring trap** (full span measured at −66) — band narrowed, span now −16.4, default byte-identical. **13 tactics fields proven unread** by grep, marked DEAD in the type and barred from the UI by a test that fails in both directions. Legibility: the line board now prices itself ("what this board is worth", conversion measured at 4.43 pts per weighted-rating point). Coach tactics measured too: forecheck (−12.4), pass risk (+11.4), dumping (−10.3), gap control (+8.2), puck pressure (+8.0) all REAL — but **PP formation (−0.3), PK formation (−0.2), shooting, pace and hitting measure as nothing** at a ±2.7-pt floor. **Remaining:** (a) freshness at 38 pts needs a calibration pass; (b) full-sim-only tactics are inert on the default *Continue* path; (c) the five no-effect systems are a blocker the day any becomes a GM control; (d) ice time reaches development only indirectly (games dressed, not minutes).
